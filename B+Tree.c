//
// Created by matheus on 08/07/18.
//
...
TABP* Busca(TABP* x, char* ch){
    ...
    if(i < x->nc && (strcmp(ch , x->chave[i])==0) && x->f){
//        printf("folha\n");
       printf("qntd de faixas : %d \n nome do album %a , \n duracao total %d",x->qf[i],x->na,x->d[i]) ;
        return x;
    }
    if(x->f) return resp;
...
//
                ...
                for(j=i; j < p->nc-1; j++){ //limpar refer�ncias de i
                    strcpy(p->chave[j],p->chave[j+1]);
